...
func (s *cfKeyring) openKeyring() (keyring.Keyring, error) {
	...
	name := os.Getenv("COMMONFATE_PROVIDER_REGISTRY_KEYRING_NAME")
	if name == "" {
		name = "commonfate_provider_registry"
	}

	dirname := os.Getenv("COMMONFATE_PROVIDER_REGISTRY_KEYRING_FILE_DIR")
	if dirname == "" {
		dirname = "~/.commonfate"
	}

	keychainName := os.Getenv("COMMONFATE_PROVIDER_REGISTRY_KEYRING_MACOS_KEYCHAIN_NAME")
	if keychainName == "" {
		keychainName = "login"
	}
	...
	kab := os.Getenv("COMMONFATE_PROVIDER_REGISTRY_KEYRING_ALLOWED_BACKENDS")
	if kab != "" {
	...
	if strings.ToLower(os.Getenv("COMMONFATE_PROVIDER_REGISTRY_KEYRING_DEBUG")) == "true" {
		keyring.Debug = true
	}
